...
sec_email = st.sidebar.text_input("SEC EDGAR Email", value="ValueApp user@example.com", 
                                  help="The SEC API requires an email address as your user agent.")
...
if analyze_btn or ticker_input:
    with st.spinner(f"Analyzing {ticker_input.upper()}..."):
        try:
            ...
            results = perform_fundamental_analysis(resolved_ticker, sec_email)
            ...
            # Edgartools Check
            edgar_status_color = "green" if results["Edgar Fetched"] else "red"
            edgar_status_text = "API Connected" if results["Edgar Fetched"] else "Skipped / Failed"
            col8.markdown(styled_metric("SEC Edgar Status", edgar_status_text, edgar_status_color), unsafe_allow_html=True)
